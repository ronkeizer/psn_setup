...
print "\nThis is an adaptation (Ron Keizer, 2014) of the original PsN installer\n".
    "to be used for scripted installation of PsN. Please be aware that it uses\n".
    "all defaults as specified by PsN, and assumes all required Perl modules are\n".
    "installed. This will install PsN version $version.\n";
...
if ($Config{osname} eq 'linux' or $Config{osname} eq 'darwin') {
	...
	if ($user =~ /^root$/) {
		...
		print "You need root privileges to install PsN systemwide.\n";
		exit;
	}
} else {
	if (not running_on_windows()) {
